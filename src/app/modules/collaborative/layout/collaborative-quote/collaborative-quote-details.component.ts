import { Component, OnInit, ViewChild, TemplateRef, NgZone, ChangeDetectorRef, OnDestroy, ViewEncapsulation, ElementRef, Renderer2 } from '@angular/core';
...
import {
  ...
  CartService, Cart, DateFormat, CollaborationRequestService, CollaborationRequest,
  CollaborationAuthenticationType, CollaborationAccessType
...
@Component({
  selector: 'app-collaborative-quote-details',
  templateUrl: './collaborative-quote-details.component.html',
  styleUrls: ['./collaborative-quote-details.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CollaborativeQuoteDetailsComponent implements OnInit, OnDestroy {
  ...
  collaborationAccessType: CollaborationAccessType | null = null;
  ...
  ngOnInit() {
    ...
    this.validateCollaborationAndLoadQuote();
  ...
  private validateCollaborationAndLoadQuote(): void {
    const quoteId = this.activatedRoute.snapshot.params['id'];
    ...
    this.quoteSubscription.push(
      combineLatest([
        this.userService.isLoggedIn().pipe(take(1)),
        this.collaborationService.getCollaborationRequest('Proposal', quoteId).pipe(take(1))
      ]).subscribe(([isLoggedIn, request]) => {
        ...
        this.collaborationAccessType = request.AccessType;
        
        if (request.AccessType !== CollaborationAccessType.AcceptReject) {
          this.exceptionService.showError(this.translateService.instant('COLLABORATION.INSUFFICIENT_ACCESS'));
          this.router.navigate(['/']);
          return;
        }
